 ...
 * @brief Plain data types the dashboard sends over HTTP/WebSocket. See
 * docs/Dashboard.md for the full type list and how each is used.
 *
 * Field names are a 1:1 match with what dashboard/js/app.js reads
 * (setStat/setInfo/setHealth/setStatus calls) -- rename in both places
 * together.
 */
...
/// There's only one payload panel (see #payload-panel in index.html, a
/// fixed container rather than an id-keyed grid), so no `id` field here.
...
/// see CatapultLauncher::setStatusCallback()). `id` matches the
/// CatapultEndpoint id from config, stringified (e.g. "1", "2").
...
struct NmpcTelemetrySnapshot {
    ...
    /// Lets the dashboard tell "the trajectory I'm showing is stale" apart
    /// from "freshly generated/applied". Wall-clock ms; 0 = nothing loaded
    /// yet this run.
    uint64_t trajectoryLoadedAtMs = 0;
    int numUavs = 0;
    bool hasPayload = false;

    /// Control-loop period (1000/hlcFrequency), sent so the frontend can
    /// judge lastSolveMs against the real deadline and size its own
...
struct TrajectoryPointJson {
    ...
    /// Mission time [s], shared across every vehicle's point at the same
    /// array index lets the dashboard plot speed/roll/pitch vs. time.
    /// Exact for a freshly-generated preview; for the post-Apply replay
    /// (read back from the solver's flattened reference, with no
    /// timestamps of its own) it's approximated as
    /// `index * TrajectoryConfig::simDt
    double time = 0.0;
...
/// Origin + reference trajectory, read once by the setup/orientation 3D
/// view via GET /api/origin and GET /api/trajectory. Not pushed over the
/// WebSocket like everything else here, since neither changes at
/// telemetry rates.
...
struct TrajectoryGenerationParams {
    ...
    /// @name Real launch-position seeding (phase angles)
    /// Fixed at two aircraft. These are LOCAL/pre-field-heading-rotation
    /// angles -- applyFieldCalibration() rotates the whole mission
    /// (including these) by fieldHeadingDeg afterward, so the real-world
    /// bearing an operator measures on site is
    /// `localPhaseDeg + fieldHeadingDeg` (see trajectoryGenerator.cpp's
    /// rotationZ convention); the capture button in setup3d.html does that
    /// conversion, not this struct.
    /// @{
    double uav1PhaseDeg = 0.0;
    double uav2PhaseDeg = 180.0;
    /// @}

    /// @name Slip-clutch tether payout
    /// Defaults equal tetherLengthMeters, i.e. no payout modeled until set
    /// shorter than tetherLengthMeters.
    /// @{
    ...
    /// -> UAV offset instead of requiring a manual guess.
    double aircraftZ0Meters = 1.382;

    /// When true, rigidly translates each UAV's whole trajectory so its
    /// launch waypoint lands exactly on that UAV's *captured* position
    /// below (see TrajectoryGenerator::snapToLiveLaunchPositions()).
    /// Position-only; defaults to false (strict no-op). Deliberately does
    /// NOT re-read live telemetry at generate/apply time -- the operator
    /// clicks "Capture live positions" once, and that snapshot is what
    /// applies on every subsequent Generate/Apply until re-captured, so
    /// this never silently re-samples a UAV that's since moved.
    bool snapToLiveLaunchPosition = false;

    /// @name Per-UAV captured launch position
    /// NED, NavigationFrameManager frame. Populated client-side by
    /// "Capture live positions" in setup3d.html. `uavNSnapCaptured=false`
    /// means "never captured" -- that UAV's launch point is left untouched
    /// even when snapToLiveLaunchPosition is true.
    /// @{
...
/// One live vehicle fix -- real GPS-derived NED position (+ yaw for UAVs)
/// at the moment "Capture live positions" was clicked. Same NED frame as
/// everything else here (relative to the NavigationFrameManager origin),
/// so it's directly comparable to TrajectoryPointJson / OriginSnapshot
/// without conversion.
...
/// `available` is false until the nav frame has a GPS-derived offset for
/// at least one UAV (see NavigationFrameManager::isInitialized()) -- e.g.
/// before GPS lock, or before controlMode==MPC has even started.
/// `hasPayload` is separate: the payload may have its own real GPS link
/// but not be reporting yet even while the UAVs are.
