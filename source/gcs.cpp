...
TrajectorySnapshot GroundControlStation::m_buildTrajectorySnapshotFromController() const {
    ...
    // The controller's flattened reference array carries no timestamps of
    // its own (see the follow-up note on solver dt in the ADR-001 status
    // doc), so approximate sample spacing with TrajectoryConfig's default
    // simDt -- exact for anything generated by TrajectoryGenerator (the only
    // path today), good enough for the time-series plot either way.
    const double assumedSimDt = grs::trajgen::TrajectoryConfig{}.simDt;
    ...
    for (int i = 0; i < numUavs; ++i) {
        ...
        for (const auto& p : m_controlInterface->getTrajectoryForVehicle(i)) {
            ...
            pt.time = static_cast<double>(idx++) * assumedSimDt;
...
TrajectorySnapshot GroundControlStation::m_missionToTrajectorySnapshot(const grs::trajgen::GeneratedMission& mission,
    ...
    if (includePayload && !mission.payload.empty()) {
        ...
        for (size_t s = 0; s < mission.payload.size(); ++s) {
            const auto& k = mission.payload[s];
            TrajectoryPointJson pt;
            pt.north = k.pos[0]; pt.east = k.pos[1]; pt.down = k.pos[2];
            pt.vx = k.vel[0]; pt.vy = k.vel[1]; pt.vz = k.vel[2];
